...
export function KelasCard() {
    ...
    const getKelas = useQuery({
        queryKey: ["kelas", page],
        queryFn: async () => {
            return await getCourses(page);
        },
    });

    useEffect(() => {
        if (getKelas.isSuccess && getKelas.data.success) {
            setCourseData(getKelas.data.docs);
        }
    ...
    return (
        <main className="w-full">
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 justify-items-center">
                {courseData &&
                    courseData.map((arr, key) => (
                        ...
                        >
                            <Card
                                className="w-full h-full bg-white/90 dark:bg-zinc-900/90 shadow-xl rounded-2xl border-0 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                                <CardHeader>
                                    ...
                                    <CardTitle
                                        className="line-clamp-3 h-20 text-lg font-bold text-gray-800 dark:text-white">
                                        {arr?.name}
                                    </CardTitle>
                      ...
                      {arr.modules.length} modul
                                    ...
                                    <CardDescription
                                        className=" text-ellipsis line-clamp-4 text-gray-600 dark:text-gray-400">
                                ...
                                <CardContent>
                                    <div className="flex items-center gap-2">
                                        {arr.price > 0 ? (
                                            <span
                                                className="text-indigo-600 dark:text-indigo-300 font-semibold text-base">
            ...
            {getKelas.isSuccess && getKelas.data.totalPages > 1 && (
                    ...
                    <PaginationContent>
                            ...
                            <PaginationPrevious
                                className={cn({
                                    "pointer-events-none opacity-50":
                                        !getKelas.data.hasPrevPage,
                                })}
                                href={changePosition(getKelas.data.prevPage)}
                            />
                        </PaginationItem>
                        {Array.from(
                            {length: getKelas.data.totalPages},
                            (_, i) => i + 1,
                        ).map((arr, index) => (
                            <PaginationItem
                                className={cn({
                                    "bg-indigo-100 dark:bg-indigo-800 pointer-events-none rounded-sm":
                                        getKelas.data.page === arr,
                                })}
                                key={index}
                            >
                        ...
                        <PaginationItem>
                            <PaginationNext
                                className={cn({
                                    "pointer-events-none opacity-50":
                                        !getKelas.data.hasNextPage,
                                })}
                                href={changePosition(getKelas.data.nextPage)}
                            />
                        </PaginationItem>
                    </PaginationContent>
                </Pagination>
