...
import {checkCouponCode} from "@/action/check-coupon.action";
import {createOrderAction} from "@/action/create-order.action";
...
export default function CheckoutPage() {
    ...
    const checkCoupon = useMutation({
        mutationFn: async ({coupon_code}: { coupon_code: string }) => {
            return await checkCouponCode(coupon_code);
        },
    });
    ...
    const createOrder = useMutation({
        mutationFn: async ({
            ...
            return await createOrderAction(course_item, order_number, coupon_code);
        },
    });
    ...
    return (
        <div
            className="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-white dark:from-gray-900 dark:via-gray-950 dark:to-gray-900 flex items-center justify-center p-4 sm:p-6">
            <motion.div
            ...
            >
                <Card className="bg-white/90 dark:bg-zinc-900/90 shadow-2xl rounded-2xl overflow-hidden border-0">
                    ...
                    <CardHeader className="pb-4 sm:pb-6">
                        <CardTitle
                            className="text-2xl sm:text-3xl font-extrabold text-gray-800 dark:text-white tracking-tight flex items-center gap-2">
                    ...
                    <CardContent className="space-y-6 text-gray-800 dark:text-gray-200">
                      ...
                      {data?.price ? toIDRFormat(data.price) : "Memuat..."}
                                ...
                                <div
                                    className="flex justify-between font-bold text-base sm:text-lg pt-1 sm:pt-2 border-t border-gray-100 dark:border-gray-800">
                                    <span>Total</span>
                                    <span>
                      {data?.price
                          ? toIDRFormat(
